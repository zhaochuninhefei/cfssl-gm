...
func TestPrepareTestData(t *testing.T) {
	...
	token, _ := testProvider.Token(req.Request)
	fmt.Println(token)
	req.Token = token
	data, _ := json.Marshal(req)
	ioutil.WriteFile("testdata/preparerequest.json", data, os.FileMode(0644))
...
func newProvider() {
	testProvider, _ = New(testKey, nil)
	testProviderAD, _ = New(testKey, testAD)
}
...
func loadBenchmarkRequest() {
	bmRequest, _ = ioutil.ReadFile("testdata/request.json")
}

func BenchmarkToken(b *testing.B) {
	newProvider()
	loadBenchmarkRequest()
	for i := 0; i < b.N; i++ {
